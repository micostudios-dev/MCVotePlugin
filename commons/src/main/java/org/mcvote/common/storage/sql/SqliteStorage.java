...
public final class SqliteStorage implements VoteStorage {
    ...
    public void init() throws Exception {
        ...
        hikari.setPoolName("MCVote-SQLite");
        ...
        try (Connection connection = dataSource.getConnection();
             PreparedStatement statement = connection.prepareStatement(
                     "INSERT INTO mcvote_players (username, display_name, uuid, streak, best_streak, total_votes, last_vote_day, last_vote_ms) " +
                             "VALUES (?, ?, ?, ?, ?, ?, ?, ?) " +
                             "ON CONFLICT(username) DO UPDATE SET display_name = excluded.display_name, uuid = excluded.uuid, " +
                             "streak = excluded.streak, best_streak = excluded.best_streak, total_votes = excluded.total_votes, " +
                             "last_vote_day = excluded.last_vote_day, last_vote_ms = excluded.last_vote_ms")) {
        ...
        try (Connection connection = dataSource.getConnection();
             PreparedStatement statement = connection.prepareStatement(
                     "INSERT INTO mcvote_players (username, display_name, uuid) VALUES (?, ?, ?) " +
                             "ON CONFLICT(username) DO UPDATE SET uuid = excluded.uuid, display_name = excluded.display_name")) {
    ...
    private static final String[] SCHEMA = {
            "CREATE TABLE IF NOT EXISTS mcvote_players (" +
                    ...
                    "last_vote_ms INTEGER NOT NULL DEFAULT 0)",
            ...
            "CREATE INDEX IF NOT EXISTS idx_deliveries_user_claimed ON mcvote_deliveries (username, claimed)",
            "CREATE TABLE IF NOT EXISTS mcvote_vote_history (" +
                    "id INTEGER PRIMARY KEY AUTOINCREMENT, " +
                    "username TEXT NOT NULL, " +
                    "service TEXT NOT NULL, " +
                    "voted_ms INTEGER NOT NULL)",
            "CREATE INDEX IF NOT EXISTS idx_history_user_service ON mcvote_vote_history (username, service)",
            "CREATE INDEX IF NOT EXISTS idx_history_user_time ON mcvote_vote_history (username, voted_ms)",
            "CREATE TABLE IF NOT EXISTS mcvote_party (" +
                    "id INTEGER PRIMARY KEY, " +
                    "progress INTEGER NOT NULL DEFAULT 0, " +
                    "total_parties INTEGER NOT NULL DEFAULT 0)",
            "INSERT OR IGNORE INTO mcvote_party (id, progress, total_parties) VALUES (1, 0, 0)"
    };
}
